// Cymbal strike animation: 5 frames, played forward then backward
// Matching CymbalPlayPose.java

function getStrikePose(context) {
    var frame = context.getInt("strikeFrame", 0);
    var dir = context.getInt("strikeDir", 1); // 1 = forward, -1 = backward
    var maxFrame = 4;
    ...
    // The hardcoded constructor increments before storing each frame, so the
    // first strike is +0.06 and the peak is +0.30.
