// Piano play pose: maps MIDI notes to hand positions on the keyboard
// Equivalent to PianoPlayPose.java
...
function getPlayPose(context) {
    ...
    // Shift notes down by 21 (A0 offset) — equivalent to stream().map(i -> i-=21)
    var shifted = [];
    ...
    if (shifted.length < 5) {
        ...
        // Split into left (<= average) and right (> average)
        ...
        // Equivalent to orElse(0) — always produce a value, never null
...
// Model pose: animate key bones based on currently playing notes
// Called at render time, independent of player arm animation
