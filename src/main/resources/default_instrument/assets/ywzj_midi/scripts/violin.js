// Violin play pose with bowing animation and pizzicato
...
function getPlayPose(context) {
    ...
    if (variantId === 0) {
        ...
        var bowRot = -0.8 + (newStep / (PULL_PUSH_STEPS - 1)) * 0.5;
        var bias = (bowRot + 0.8) / 0.5;

        var bowBuilder = createPoseBuilder();
        bowBuilder.setLeftArmRotation(-1.2, null, -0.8);
        bowBuilder.setRightArm(
            -1.75 + bias * 0.25, bowRot, 0,
            -4.0 - bias * 2, null, -1.5 + bias * 2
        );
        return bowBuilder;
    } else {
        ...
        pizzBuilder.setLeftArmRotation(-1.2, null, -0.8);
